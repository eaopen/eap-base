...
import static org.openea.eap.extj.consts.AuthConsts.DEF_TENANT_DB;
...
public abstract class AbstractTokenGranter implements TokenGranter, Ordered {
     ...
     * @return
     */
    protected String getExtnTicket(){
        return SaHolder.getRequest().getParam(AuthConsts.PARAMS_EXTN_TICKET, "");
    }

    protected boolean isValidExtnTicket(){
        String extnTicket = getExtnTicket();
        if(!extnTicket.isEmpty()){
            LoginTicketModel loginTicketModel = TicketUtil.parseTicket(extnTicket);
