...
//            " cast(COLUMNPROPERTY(a.id,a.name,'PRECISION') as varchar(50)) " + DbAliasConst.DATA_LENGTH + ", " +
    ...
    ORDER_PAGE     ("SELECT * " +
                            "FROM " +
                                "(SELECT TOP {endIndex} ROW_NUMBER() OVER(ORDER BY {orderColumn}) EXTN_ROW,* " +
                            "FROM " +
                                "({selectSql}) AS EXTN_TAB ORDER BY EXTN_ROW) AS EXTN_TEMP_TABLE " +
                            "WHERE " +
                                "EXTN_ROW BETWEEN {beginIndex} AND {endIndex}"),
