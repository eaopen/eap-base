...
public abstract class DbBase {
     ...
     * 指定子类被创建时，需要提供的参数
     */
    protected void setInstance(String extnDbEncode, DbType mpDbType, com.alibaba.druid.DbType druidDbType, String defaultPort, String dbaUsername, String connUrlEncode,
                               String driver, String defaultPrepareUrl) {
        // 绑定：EXTN数据库编码
        this.extnDbEncode = extnDbEncode;
        // 绑定：MybatisPlus数据库编码
...
//                return hasDataSource ? getDynamicTableName(tableName) : tableName;
    ...
    public void setPartFieldModel(DbFieldModel model, ResultSet result) throws Exception{
        model.setDtModelDTO(new DtModelDTO(
                DtInterface.newInstanceByDt(model.getDataType(), this.getExtnDbEncode()),
                result.getLong(DbAliasEnum.CHAR_LENGTH.getAlias(extnDbEncode)),
                result.getInt(DbAliasEnum.NUM_PRECISION.getAlias(extnDbEncode)),
                result.getInt(DbAliasEnum.NUM_SCALE.getAlias(extnDbEncode))
        ));
    }
